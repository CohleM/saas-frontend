...
const Tiptap = () => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "max-w-full prose dark:prose-invert prose-sm sm:prose-base lg:prose-base xl:prose-base m-5 focus:outline-none",
      },
    },
    onUpdate: (e) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const editDraftResponse = await fetch(
          `http://127.0.0.1:8000/edit-draft/${activeDraft}/`,
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      if (editor) {
        if (message['type'] == 'log') {
          // print the logs
          // setLogMessage(message['content'])
          setLogMessage((prevList) => [...prevList, message['content']]);
    ...
    const fetchData = async () => {
      try {
        ...
        const draftResponse = await fetch(
          `http://127.0.0.1:8000/draft?id=${activeDraft}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const createNewDraft = () => {
    if (processing) {
      ...
      const fetchData = async () => {
        try {
          const draftResponse = await fetch(
            `http://127.0.0.1:8000/create-draft`,
            {
              method: "GET",
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  return (
    <div>
      ...
      <div className="flex gap-x-4 py-4 ">
        ...
        {toggleSidebar && drafts && (
          <div className="top-0 fixed left-0 w-[240px] h-screen overflow-y-auto border-r border-solid border-gray-50000">
            <Drafts
              contentChange={handleContent}
              drafts={drafts}
              setActiveDraft={handleActiveDraft}
        ...
        <div
          className={`w-full overflow-y-auto  mt-10 ${
            toggleSidebar ? "ml-[260px]" : "ml-10"
          } `}
        >
          {loading ? (
            <LoadingEditor />
        ...
        <div className=" sticky top-0 w-1/2  h-screen overflow-y-auto border-l border-solid border-gray-300 p-4">
          {" "}
          <div>
            {" "}
            <CheckPortal
              sendMessage={handleSendMessage}
              holdEditing={holdEditing}
            >
              {" "}
            </CheckPortal>
